...
function buildRepetition(itemRule, minItems, maxItems) {
  ...
  const optRepetitions = function (upToN) {
    let prefixWithSep = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
...
export class SchemaGrammarConverter {
  ...
  _buildObjectRule(properties, required, name, additionalProperties) {
    ...
    const sortedProps = properties.map(_ref2 => {
      let [k] = _ref2;
      return k;
    }).sort((a, b) => {
      const orderA = propOrder[a] || Infinity;
      const orderB = propOrder[b] || Infinity;
      return orderA - orderB || properties.findIndex(_ref3 => {
        let [k] = _ref3;
        return k === a;
      }) - properties.findIndex(_ref4 => {
        let [k] = _ref4;
        return k === b;
      });
    });
  ...
  formatGrammar() {
    let grammar = '';
    for (const [name, rule] of Object.entries(this._rules).sort((_ref5, _ref6) => {
      let [a] = _ref5;
      let [b] = _ref6;
...
export const convertJsonSchemaToGrammar = _ref7 => {
  let {
    schema,
    propOrder,
    dotall,
