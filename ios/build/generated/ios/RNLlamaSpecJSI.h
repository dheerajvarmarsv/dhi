...
struct NativeRNLlamaNativeCompletionParams {
  ...
  P20 dry_multiplier;
  P21 dry_base;
  P22 dry_allowed_length;
  P23 dry_penalty_last_n;
  P24 dry_sequence_breakers;
  P25 ignore_eos;
  P26 logit_bias;
  P27 seed;
  P28 emit_partial_completion;
  bool operator==(const NativeRNLlamaNativeCompletionParams &other) const {
    return prompt == other.prompt && n_threads == other.n_threads && grammar == other.grammar && stop == other.stop && n_predict == other.n_predict && n_probs == other.n_probs && top_k == other.top_k && top_p == other.top_p && min_p == other.min_p && xtc_probability == other.xtc_probability && xtc_threshold == other.xtc_threshold && typical_p == other.typical_p && temperature == other.temperature && penalty_last_n == other.penalty_last_n && penalty_repeat == other.penalty_repeat && penalty_freq == other.penalty_freq && penalty_present == other.penalty_present && mirostat == other.mirostat && mirostat_tau == other.mirostat_tau && mirostat_eta == other.mirostat_eta && dry_multiplier == other.dry_multiplier && dry_base == other.dry_base && dry_allowed_length == other.dry_allowed_length && dry_penalty_last_n == other.dry_penalty_last_n && dry_sequence_breakers == other.dry_sequence_breakers && ignore_eos == other.ignore_eos && logit_bias == other.logit_bias && seed == other.seed && emit_partial_completion == other.emit_partial_completion;
  }
};
...
#pragma mark - NativeRNLlamaNativeLlamaChatMessage

template <typename P0, typename P1>
struct NativeRNLlamaNativeLlamaChatMessage {
  P0 role;
  P1 content;
  bool operator==(const NativeRNLlamaNativeLlamaChatMessage &other) const {
    return role == other.role && content == other.content;
  }
};

template <typename T>
struct NativeRNLlamaNativeLlamaChatMessageBridging {
  static T types;
  ...
  static T fromJs(
      ...
      const std::shared_ptr<CallInvoker> &jsInvoker) {
    T result{
      bridging::fromJs<decltype(types.role)>(rt, value.getProperty(rt, "role"), jsInvoker),
      bridging::fromJs<decltype(types.content)>(rt, value.getProperty(rt, "content"), jsInvoker)};
    return result;
  }

#ifdef DEBUG
  static jsi::String roleToJs(jsi::Runtime &rt, decltype(types.role) value) {
    return bridging::toJs(rt, value);
  }
  ...
  static jsi::Object toJs(
      ...
      const std::shared_ptr<CallInvoker> &jsInvoker) {
    auto result = facebook::jsi::Object(rt);
    result.setProperty(rt, "role", bridging::toJs(rt, value.role, jsInvoker));
    result.setProperty(rt, "content", bridging::toJs(rt, value.content, jsInvoker));
    return result;
  }
};
...
public:
  ...
  virtual jsi::Value getFormattedChat(jsi::Runtime &rt, double contextId, jsi::Array messages, std::optional<jsi::String> chatTemplate) = 0;
...
private:
  ...
  public:
    ...
    jsi::Value getFormattedChat(jsi::Runtime &rt, double contextId, jsi::Array messages, std::optional<jsi::String> chatTemplate) override {
      static_assert(
          bridging::getParameterCount(&T::getFormattedChat) == 4,
          "Expected getFormattedChat(...) to have 4 parameters");

      return bridging::callFromJs<jsi::Value>(
          rt, &T::getFormattedChat, jsInvoker_, instance_, std::move(contextId), std::move(messages), std::move(chatTemplate));
    }
